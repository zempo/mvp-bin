...
describe(`Comment Endpoints`, () => {
  ...
  describe(`POST a comment to /api/comments/`, () => {
      ...
      it("Creates a new comment, responding with 201 and the new comment", function () {
        ...
        return supertest(app)
          ...
          .expect((res) => {
            ...
            expect(res.body.payload.body).to.eql(newComment.body);
            expect(res.body.payload.user.id).to.eql(testUser.id);
            expect(res.headers.location).to.eql(
              `/api/comments/${res.body.payload.id}`
            );
            const expectedDate = new Date().toLocaleString("en", {
              timeZone: "America/Los_Angeles",
            });
            const actualDate = new Date(
              res.body.payload.date_created
            ).toLocaleString();
    ...
    context(`Authorized user posts a profane comment`, () => {
      ...
      beforeEach("insert cards", () =>
        helpers.seedCardsTables(db, testUsers, testCards)
      );
      ...
      it("Creates a new comment, responding with 201 and the censored body", function () {
        ...
        return supertest(app)
          ...
          .expect(201)
          .expect((res) => {
            expect(res.body.payload.body).to.eql("Testing **** comment");
          });
      });
    });
  });
  ...
  describe(`GET api/comments/comment_id`, () => {
    ...
    context("Given a comment exists", () => {
      ...
      it("Responds with 200 for valid user", () => {
        const commentToGet = 2;
        const expectedComment = helpers.makeExpectedComments(
          testUsers,
          commentToGet,
          testComments
        )[0];
        ...
        return supertest(app)
          ...
          .expect(200, {
            success: true,
            message: `Showing comment with id of ${commentToGet}.`,
    ...
    context("Given an invalid patch", () => {
      it("Responds with 403 for invalid/nonadmin user", () => {
        ...
        return supertest(app)
          ...
          .expect(403, {
            success: false,
            message: "User does not match card.",
          });
      });
      ...
      it("Responds with 404 for insufficient fields updated", () => {
        ...
        return supertest(app)
          ...
          .expect(400, {
            success: false,
            message: "Missing required 'body' to create new comment.",
          });
      });
    });
